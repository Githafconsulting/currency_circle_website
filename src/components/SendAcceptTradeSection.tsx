...
const SendAcceptTradeSection = () => {
  const { t } = useTranslation()
  return (
    <section id="accept-trade" className="relative py-12 sm:py-16 lg:py-32 bg-black overflow-hidden">
      {/* Rounded separator */}
        ...
        <div className="flex flex-col lg:flex-row items-center justify-center gap-8 lg:gap-8">
          ...
          <div className="max-w-xl space-y-4 sm:space-y-6 animate-slide-from-center-left lg:order-1 order-2 text-center lg:text-right">
            <div className="lg:flex lg:justify-end">
              <span className="bg-green-500/20 text-white px-3 sm:px-4 py-2 rounded-full text-xs sm:text-sm font-semibold border border-green-500/30 inline-block">
                {t('sendAccept.badge')}
              </span>
            </div>
            ...
            <h2 className="text-2xl sm:text-3xl lg:text-4xl xl:text-5xl font-bold text-white leading-tight">
              {t('sendAccept.title')}
              <span className="block text-gradient mt-2">{t('sendAccept.titleGradient')}</span>
            </h2>

            <p className="text-base sm:text-lg text-gray-300 leading-relaxed">
              {t('sendAccept.description')}
            </p>
          </div>

          {/* Vertical Divider Line */}
          <div className="hidden lg:block relative h-96 mx-4 lg:order-2 order-1">
            <div className="absolute inset-0 w-px bg-gradient-to-b from-transparent via-green-500 to-transparent opacity-50"></div>
          ...
          <div className="flex justify-center lg:justify-start animate-slide-from-center-right lg:order-3 order-1">
            <div className="relative">
              {/* Glow effect behind phone */}
              <div className="absolute inset-0 bg-gradient-to-br from-green-500/30 to-primary/30 rounded-full blur-3xl scale-75" />

              {/* Phone mockup with animation */}
              <div className="relative transform hover:scale-105 transition-transform duration-500">
                <img
                  src="/trade.png"
                  alt="Accept Trade Request Screen"
...
export default SendAcceptTradeSection
